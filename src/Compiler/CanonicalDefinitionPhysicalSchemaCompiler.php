...
namespace Kumwe\BusinessSchema\Compiler;
 ...
 * @since  2.0.0
 */
final readonly class CanonicalDefinitionPhysicalSchemaCompiler
{
    /**
    ...
    public function __construct(
        private DefinitionSchemaLookup $definitions,
     ...
     * @since   2.0.0
     */
    public function compile(EntityTypeDefinition $definition, string $site): PhysicalSchemaBlueprint
    {
        if ($definition->siteIdentifier !== $site || $definition->definitionVersion < 1) {
     ...
     * @param   string           $site        Site in which reference and relationship targets are
     *          resolved.
     *
     ...
     * @since   2.0.0
     */
    private function recordTable(EntityTypeDefinition $definition, string $site): PhysicalTableBlueprint
    {
     ...
     * @param   string           $site    Site both definitions belong to.
    ...
    private function targetFor(
        ...
        $target = $this->definitions->published($site, $handle, $version);
        if (
